...
datasix.visualization = (function(visualization)
{
  ...
  /**
   * @name datasix.visualization.Map
   * @class Map
   * @param {object} container_ref  A reference to a valid container element on the page. The chart will be drawn there. 
   * @example https://google-developers.appspot.com/chart/interactive/docs/gallery/geochart
   */
  visualization.Map = function(container_ref)
  {
    ...
    /** 
     * @name setIParamValues
     * @methodOf datasix.visualization.Map
     * @description Sets object used to access parameter values
     * @param {cnd.ebis.IParamValues} An instance of an object implementing the IParamValues interface
     */
    this.setIParamValues = function(iParamValues) 
   ...
   /** 
    * @name getContainer
    * @methodOf datasix.visualization.Map.prototype
    * @returns {object} 
    */
    ...
    **/
    ...
    {
      var objectRet = {formattedValue: false,
                       col: undefined};
      ...
      try
      {
         if (data !== undefined)
        {
          ...
          for(var i=0; i < data.getNumberOfRows(); i++) 
          {
            switch(dataLocalisation)
            {
              case 'latlng':
                if (bShowMarkers) 
                {
                  locObj.markers[i] = {lat: data.getValue(i, 0),
                                       lng: data.getValue(i, 1)};
                }
                if (bShowCircles) 
                {
                  locObj.circles[i] = {lat: data.getValue(i, 0),
                                       lng: data.getValue(i, 1)};
                }
                break;
              ...
              case 'address':
                getGeocodeOSM(data.getValue(i, 0), 
                             ...
                             {
                              if (bShowMarkers) {
                                locObj.markers[i] = {lat: geocode[0].lat,
                                                     lng: geocode[0].lon};
                              }

                              if (bShowCircles) {
                                locObj.circles[i] = {lat: geocode[0].lat,
                                                     lng: geocode[0].lon};
                              }
                });
          ...
          if ( bShowMarkers ) 
          {
            ...
            this.logDebug ("setMarkers (seconds): " + (sMtimeE.getTime()-sMtimeB.getTime()) / 1000 );
          }
    ...
    */
    ...
    {
      google.visualization.errors.removeAll(m_container);
      //this.logDebug ('datasix.visualization.Map.draw');
      if (m_container === undefined)
        //throw ("datasix.visualization.Map.draw container undefined.");
      {
        this.setError("datasix.visualization.Map.draw container undefined.");
        this.fireReady();
        return;
      }
      ...
      // If options undefined, set default options value here!
      if (options === undefined)
      {
        var latlng = new google.maps.LatLng(41.386839, 2.170036); //41.409775832009565, 2.4169921875
        options = 
        {
          zoom: 2,
          center: latlng,
      ...
      var divEltId = 'datasix_visualization_Map';
      ...
      if (m_map === undefined)
      {
        m_map = new google.maps.Map($divElt.get(0), options); 
        
        
        /* -- DATASIX.Dev : begin -- GMap3 (http://gmap3.net)*/
        /*
        $divElt.gmap3({
          map: options
        });
    ...
    */
    this.getSelection = function()
    {
      ...
      if (m_currentSelection !== undefined)
        arraySelection.push({row: m_currentSelection.row, column: m_currentSelection.column});
